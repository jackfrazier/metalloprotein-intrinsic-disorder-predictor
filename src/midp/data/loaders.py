...
from ..core.data_structures import (
    ProteinData, MetalSite, MetalType, Residue, ResidueType,
    Coordinates3D, MetalLigand, DisorderRegion, CoordinationGeometry
)
from ..core.exceptions import (
    StructureValidationError, FileNotFoundError,
    DataAccessError, ValidationError
)
...
METAL_ATOMS = {
    'FE', 'ZN', 'CU', 'CA', 'MG', 'MN', 'CO', 'NI', 'MO', 'W',
    'FE2', 'FE3', 'ZN2', 'CU1', 'CU2', 'CA2', 'MG2', 'MN2', 'CO2', 'NI2'
}
...
class ProteinLoader:
    ...
    def load_protein(self, 
                    ...
                    chain_ids: Optional[List[str]] = None) -> ProteinData:
        """
        ...
        logger.info(f"Successfully loaded protein {protein_id} with "
        ...
        try:
            ...
            try:
                ...
                raise DataAccessError(
                    "RCSB PDB", pdb_id, "download", 
                    f"Failed to download: {str(e2)}"
                )
    ...
    def _parse_pdb_file(self, pdb_file: Path) -> Structure:
        ...
        try:
            structure = self.pdb_parser.get_structure(
                pdb_file.stem, str(pdb_file)
            )
            return structure
        except Exception as e:
            raise StructureValidationError(
                str(pdb_file), f"Failed to parse: {str(e)}"
            )
    
    def _parse_mmcif_file(self, cif_file: Path) -> Structure:
        """Parse an mmCIF file using BioPython."""
        try:
            structure = self.mmcif_parser.get_structure(
                cif_file.stem, str(cif_file)
            )
            return structure
        except Exception as e:
            raise StructureValidationError(
    ...
    def _extract_sequence_and_residues(self, 
                                      structure: Structure,
        ...
        for model in structure:
            for chain in model:
                ...
                for bio_residue in chain:
                    ...
                    if not is_aa(bio_residue, standard=True):
                        continue
                    
                    # Get residue information
                    resname = bio_residue.get_resname()
                    ...
                    try:
                        ...
                        logger.warning(f"Non-standard amino acid {resname}")
                        residue_type = ResidueType.ALA  # Default
        ...
        if not sequence:
            raise StructureValidationError(
                structure.id, "No amino acid residues found"
            )
        
        return sequence, residues, chains_found
    
    def _detect_metal_sites(self, structure: Structure, residues: List[Residue]) -> List[MetalSite]:
        ...
        for model in structure:
            ...
            for chain in model:
                for residue in chain:
                    for atom in residue:
                        element = atom.element.upper() if atom.element else atom.name[:2]
            ...
            for metal_atom, metal_res, metal_chain in metal_atoms:
                ligands = self._find_metal_ligands(
                    metal_atom, model, metal_res.id[1]
                )
                ...
                element = metal_atom.element.upper() if metal_atom.element else metal_atom.name[:2]
                ...
                for lig_atom, lig_res, lig_chain, distance in ligands:
                    ...
                    residue_match = None
                    for res in residues:
                        if (res.chain_id == lig_chain.id and 
                            res.position == lig_res.id[1]):
                            residue_match = res
                            break
    ...
    def _find_metal_ligands(self, 
                           metal_atom: PDB.Atom.Atom,
                           model: PDB.Model.Model,
                           metal_resnum: int) -> List[Tuple]:
    ...
    def _determine_coordination_geometry(self,
                                       metal_atom: PDB.Atom.Atom,
    ...
    def _calculate_geometry_rmsd(self,
                               metal_atom: PDB.Atom.Atom,
                               ligand_atoms: List[PDB.Atom.Atom],
                               geometry: CoordinationGeometry) -> float:
    ...
    def _predict_disorder_regions(self, 
                                 sequence: str,
    ...
    def _validate_protein_data(self, protein_data: ProteinData):
        ...
        invalid_positions = [
            i + 1 for i, aa in enumerate(protein_data.sequence)
        ...
        if protein_data.resolution is not None:
            if (protein_data.resolution < VALIDATION_LIMITS['min_structure_resolution'] or
                protein_data.resolution > VALIDATION_LIMITS['max_structure_resolution']):
                logger.warning(
                    f"Unusual resolution: {protein_data.resolution} Å"
                )
        
        # Check for missing residues
        if protein_data.has_structure:
            n_missing = sum(1 for res in protein_data.residues if res.coordinates is None)
