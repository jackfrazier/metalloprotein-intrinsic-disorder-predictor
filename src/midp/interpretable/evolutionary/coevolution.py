...
from ...core.constants import COEVOLUTION_PARAMETERS
from ...core.data_structures import MetalSite
...
class CoevolutionAnalyzer:
    ...
    def calculate_coevolution_matrix(self,
                                   msa: MultipleSeqAlignment,
                                   seq_weights: Optional[np.ndarray] = None,
                                   metal_sites: Optional[List[MetalSite]] = None) -> np.ndarray:
        """
    ...
    def _calculate_mi_apc(self,
        ...
        for i in range(n_positions):
            for j in range(i + 1, n_positions):
                ...
                joint_freq = self._calculate_joint_frequencies(
                    msa, i, j, seq_weights
                )
                ...
                for a in range(self.n_states):
                    for b in range(self.n_states):
                        if (single_freqs[i, a] > 0 and 
                            single_freqs[j, b] > 0 and
                            joint_freq[a, b] > 0):
    ...
    def _calculate_single_site_frequencies(self,
                                         msa: MultipleSeqAlignment,
                                         seq_weights: np.ndarray) -> np.ndarray:
    ...
    def _calculate_joint_frequencies(self,
                                   msa: MultipleSeqAlignment,
                                   pos1: int,
                                   pos2: int,
                                   seq_weights: np.ndarray) -> np.ndarray:
    ...
    def _apply_apc(self, mi_matrix: np.ndarray) -> np.ndarray:
        ...
        for i in range(n_positions):
            ...
            for j in range(max(0, i - self.min_separation + 1),
    ...
    def _calculate_mi_significance(self,
    ...
    def _calculate_dca_scores(self,
                            msa: MultipleSeqAlignment,
                            seq_weights: Optional[np.ndarray] = None) -> np.ndarray:
        """
    ...
    def _enhance_metal_coevolution(self,
    ...
    def _check_coordinated_conservation(self,
                                      msa: MultipleSeqAlignment,
                                      pos1: int,
                                      pos2: int) -> bool:
    ...
    def identify_coevolution_networks(self,
                                    coevo_matrix: np.ndarray,
                                    threshold: Optional[float] = None) -> List[Set[int]]:
        """
    ...
    def calculate_sector_decomposition(self,
                                     coevo_matrix: np.ndarray,
                                     n_sectors: int = 3) -> Dict[int, List[int]]:
        """
