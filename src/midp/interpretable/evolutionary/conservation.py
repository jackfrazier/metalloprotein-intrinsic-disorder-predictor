...
class ConservationAnalyzer:
    ...
    def __init__(self,
        ...
        if use_blosum:
            if blosum_matrix == "blosum62":
                self.subst_matrix = MatrixInfo.blosum62
            elif blosum_matrix == "blosum80":
                self.subst_matrix = MatrixInfo.blosum80
            else:
                self.subst_matrix = MatrixInfo.blosum50
    
    def calculate_conservation_scores(self,
                                    msa: MultipleSeqAlignment,
                                    seq_weights: Optional[np.ndarray] = None,
                                    metal_positions: Optional[List[int]] = None) -> np.ndarray:
        """
        ...
        # Combine scores
        combined_scores = (
            (1 - self.property_weight) * entropy_scores +
            self.property_weight * property_scores
        )
    ...
    def _calculate_entropy_conservation(self,
                                      msa: MultipleSeqAlignment,
                                      seq_weights: Optional[np.ndarray] = None) -> np.ndarray:
    ...
    def _calculate_property_conservation(self,
                                       msa: MultipleSeqAlignment,
                                       seq_weights: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        for pos in range(n_positions):
            ...
            property_scores[pos] = (
                0.6 * max_prop_freq +
                0.2 * metal_binding_freq +
                0.2 * cysteine_freq
            )
        
        return property_scores
    
    def _apply_blosum_smoothing(self,
                               msa: MultipleSeqAlignment,
        ...
        for pos in range(n_positions):
            ...
            for i in range(len(aa_list)):
                for j in range(i, len(aa_list)):
                    ...
                    # Get BLOSUM score
                    pair = (aa1, aa2) if (aa1, aa2) in self.subst_matrix else (aa2, aa1)
                    if pair in self.subst_matrix:
                        score = self.subst_matrix[pair]
                        weight_product = aa_counts[aa1] * aa_counts[aa2] / (total_weight ** 2)
                        blosum_conservation += score * weight_product
                        comparisons += weight_product
            
            if comparisons > 0:
                # Normalize BLOSUM score (typical range -4 to 11)
                normalized_blosum = (blosum_conservation + 4) / 15
                normalized_blosum = max(0, min(1, normalized_blosum))
                
                # Blend with base score
                smoothed_scores[pos] = 0.7 * base_scores[pos] + 0.3 * normalized_blosum
        
        return smoothed_scores
    
    def _boost_metal_positions(self,
                             scores: np.ndarray,
                             metal_positions: List[int],
                             msa: MultipleSeqAlignment) -> np.ndarray:
    ...
    def identify_conserved_metal_motifs(self,
                                      msa: MultipleSeqAlignment,
                                      conservation_scores: np.ndarray) -> List[Dict[str, any]]:
        """
        ...
        for motif_name, (pattern, description) in metal_patterns.items():
            ...
            for match in re.finditer(pattern, consensus):
                ...
                if region_conservation > CONSERVATION_THRESHOLDS['moderately_conserved']:
                    motif_info = {
                        'name': motif_name,
    ...
    def calculate_relative_entropy(self,
                                 msa: MultipleSeqAlignment,
                                 background_freqs: Optional[Dict[str, float]] = None) -> np.ndarray:
        """
