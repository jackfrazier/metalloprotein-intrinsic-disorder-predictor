...
class SequenceValidationError(ValidationError):
    ...
    def __init__(self, sequence: str, invalid_positions: List[int]):
        ...
        details = {
            ...
            'invalid_chars': [sequence[i-1] for i in invalid_positions if i <= len(sequence)]
        }
...
class TrackDisagreementError(IntegrationError):
    ...
    def __init__(self, interpretable_score: float, blackbox_score: float, threshold: float):
        disagreement = abs(interpretable_score - blackbox_score)
        message = f"Track disagreement ({disagreement:.3f}) exceeds threshold ({threshold})"
        details = {
