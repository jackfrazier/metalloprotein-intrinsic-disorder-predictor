...
class MetalLigand:
    ...
    def ligand_type(self) -> str:
        ...
        atom_type_map = {
            ...
            'ND1': 'N', 'ND2': 'N', 'NE2': 'N',  # Histidine nitrogens
            'OE1': 'O', 'OE2': 'O',  # Glutamate oxygens
            'OD1': 'O', 'OD2': 'O',  # Aspartate oxygens
            'O': 'O',   # Backbone oxygen
...
class EvolutionaryFeatures:
    ...
    functional_sites: List[Tuple[int, float]] = field(default_factory=list)  # (position, score)
    ...
    def get_highly_conserved_positions(self, threshold: float = 0.8) -> List[int]:
        ...
        return [i + 1 for i, score in enumerate(self.conservation_scores) if score > threshold]
...
class FrustrationFeatures:
    ...
    frustrated_contacts: List[Tuple[int, int, float]] = field(default_factory=list)  # (i, j, frustration)
    
    def get_highly_frustrated_residues(self, threshold: float = 2.0) -> List[int]:
        """Get residues with high frustration."""
        return [i + 1 for i, fi in enumerate(self.frustration_indices) if fi > threshold]
...
class ProteinData:
    ...
    def is_valid(self) -> bool:
        ...
        return (
            len(self.sequence) > 0 and
            all(aa in 'ACDEFGHIKLMNPQRSTVWY' for aa in self.sequence)
        )
...
class InterpretableResult:
    ...
    predicted_metal_sites: List[Tuple[int, MetalType, float]]  # (position, metal, confidence)
